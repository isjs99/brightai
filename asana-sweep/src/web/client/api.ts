...
export const api = {
  ...
  bulkPreview: (opts: { market?: string; ids?: number[]; include_drafted?: boolean } = {}) => call<{ count: number; items: { prospect_id: number; shop_name: string; brand: string | null; market: string; contact_id: number; contact_name: string; contact_title: string | null; contact_email: string | null }[] }>('GET', `/bd/drafts/bulk/preview?market=${encodeURIComponent(opts.market ?? '')}${opts.ids?.length ? `&ids=${opts.ids.join(',')}` : ''}${opts.include_drafted ? '&include_drafted=1' : ''}`),
  ...
  linkedinBulk: (ids: number[], opts: { log?: boolean; include_started?: boolean } = {}) => call<BdData & { items: { prospect_id: number; shop_name: string; brand: string | null; contact_id: number; contact_name: string; contact_title: string | null; linkedin_url: string; status: string }[]; skipped: { prospect_id: number; shop_name: string; reason: string }[]; logged: number }>('POST', '/bd/linkedin/bulk', { ids, ...opts }),
  ...
  ttsContactFor: (prospectId: number) => call<{ contact: TtsContact | null; fallback: TtsContact | null; reason: string; tier: 'category' | 'tsp_manager' | 'none' }>('GET', `/bd/prospects/${prospectId}/tts-contact`),
  importTtsContactsFromGmail: () => call<OutreachData & { found: number; added: number; updated: number }>('POST', '/bd/tts-contacts/import-gmail'),
  ...
  fastmossTest: () => call<BdData & { ok: boolean; transport: 'http' | 'cli'; server: string | null; tools: number; rows: number; error: string | null }>('POST', '/bd/fastmoss/test'),
  fastmossPull: () => call<BdData & { fastmoss: BdData['fastmoss'] & { markets?: unknown }; fastmoss_error: string | null; imported: { files: string[]; added: number; updated: number } }>('POST', '/bd/fastmoss/pull'),
  fastmossSettings: (b: { markets?: string; pages?: number; enabled?: boolean; cron?: string; sorts?: string; min_gmv_7d?: number; min_rise?: number }) => call<BdData>('PUT', '/bd/fastmoss/settings', b),
