...
export async function draftWithClaude(system: string, user: string, opts: { apiKey?: string; model?: string; fetchFn?: typeof fetch } = {}): Promise<string> {
  ...
  const res = await (opts.fetchFn ?? fetch)('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model: opts.model ?? config.replyModel, max_tokens: 600, system, messages: [{ role: 'user', content: user }] }),
  });
