...
- **Daily pulls**: a scheduled Claude routine (05:30 UTC) pulls the fast risers from FastMoss each morning and commits `data/bd-pulls/YYYY-MM-DD.json` on the branch the dashboard runs from (`git pull` on the server picks it up; the server imports new files at startup and at 06:00, or on Import pull). The routine runs inside a Claude session that holds the FastMoss connector; routines created without connectors cannot call FastMoss and silently do nothing, so check the file landed after the first run. Raw `shop_search` / `shop_base_info` rows work as-is; pasting a result into Import pull does the same by hand.
