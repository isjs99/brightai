...
- **Daily pulls (FastMoss API)**: with `FASTMOSS_CLIENT_ID` / `FASTMOSS_CLIENT_SECRET` from developers.fastmoss.com the server pulls the fast risers itself every morning (05:30 Madrid by default, editable on the BD page): the top pages per market by 7-day GMV, keeping every shop with 5%+ of lifetime GMV made this week plus the top 30 for refreshed numbers, saved to `data/bd-pulls/YYYY-MM-DD.json` and upserted into the pipeline, then Apollo enrichment runs. "Test FastMoss" finds the token and shop endpoints (the developer docs were not reachable when this was built, so the paths are discoverable and editable), runs a one-row search and reads the credit balance when the API exposes it; "Pull FastMoss now" runs the pull on demand. When FastMoss runs out of credits the pull stops, the page says so, and the next scheduled pull tries again.
...
- **Decision makers**: with `APOLLO_API_KEY` set, "Find decision makers" resolves the company in Apollo (id, domain, industry, size, LinkedIn, location), searches its people twice (the TikTok / e-commerce / marketing title list, then anyone senior when that is thin), puts people based in the market first, keeps the best-ranked as contacts (10 by default) and reveals the top few for a verified work email and LinkedIn (5 by default, one credit each, no confirmation); "Reveal" does the rest one by one. Name-only matches are kept only when the person's employer matches the brand. Both numbers are settings on the BD page. Contacts can also be added by hand.
...
| `FASTMOSS_CLIENT_ID`, `FASTMOSS_CLIENT_SECRET`, `FASTMOSS_PATHS` | Optional FastMoss OpenAPI credentials so the server pulls the fast risers itself; paths override only if the docs differ |
